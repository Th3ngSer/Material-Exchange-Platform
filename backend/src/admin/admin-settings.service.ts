...
import { TrackItemUser, TrackItemUserDocument } from '../trackitemuser/schemas/trackitemuser.schema';
...
export class AdminSettingsService {
  constructor(
    @InjectModel(AdminSettings.name) private settingsModel: Model<AdminSettingsDocument>,
    @InjectModel(Post.name) private postModel: Model<PostDocument>,
    @InjectModel(Transaction.name) private transactionModel: Model<TransactionDocument>,
    @InjectModel(TrackItemUser.name) private trackModel: Model<TrackItemUserDocument>,
  ...
  async updateSettings(data: Partial<AdminSettings>): Promise<AdminSettingsDocument> {
    let settings = await this.settingsModel.findOne().exec();
    if (!settings) {
  ...
  async resetPlatformData(): Promise<{ message: string }> {
    ...
    return { message: 'Platform data (transactions, track items, reports, and activity logs) has been reset successfully.' };
  }
}
